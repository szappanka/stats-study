...
- Állandó varianciája van a hibatagoknak (homoszcedaszticitás) — a szórás nem nő vagy csökken x mentén
...
Mielőtt a képletbe mennénk, értsd meg mit mér a három négyzetösszeg:

- **SST** (Sum of Squares Total) — mennyire szórnak a valódi Y értékek az összátlag körül. Ez a "kiindulópont" — annyit tudnánk mondani Y-ról, ha nem lenne modellünk, csak az átlag.
- **SSE** (Sum of Squares Error) — mennyire téved a modell. Ez a "maradék" — amit a modell nem tudott megmagyarázni.
- **SSR** (Sum of Squares Regression) — amit a modell megmagyarázott. SST-ből kivonva az SSE-t kapjuk: SSR = SST - SSE.

Gondolj rá így: az összes "feladványból" (SST) a modell megoldotta a SSR részét, és SSE maradt megoldatlan.
...
| Mit látsz a plotban | Melyik feltétel sérül | Mit jelent / mit kell tenni |
|---|---|---|
| Ívelt mintázat (curved pattern) | linearitás | a kapcsolat nem lineáris — magasabb fokú tagot vagy más változót kell hozzáadni |
| Tölcsér alak (egyre nagyobb eltérések) | homoszcedaszticitás | a szórás nem állandó — a konfidenciaintervallumok és p-értékek pontatlanok lesznek |
| Szisztematikus hullámzás | függetlenség | autokorrelált hibák — tipikusan idősoroknál fordul elő |

Normalitást Shapiro-Wilk teszttel vagy QQ-plottal ellenőrizzük:
- Shapiro-Wilk: ha p < 0.05 → elutasítjuk a normalitást
- QQ-plot: ha a pontok nagyjából egy egyenesen vannak → normalitás teljesül

**Mini példa**

10 kaktusz éves növekedését (\\(Y\\), cm) és átlagos napi napsütését (\\(X\\), óra) vizsgáljuk.

Becsült modell: \\(\hat{Y} = 2 + 1.5x\\)

- \\(\hat\beta_1 = 1.5\\): minden plusz napsütéses óra átlagosan 1.5 cm-rel növeli az éves növekedést
- \\(\hat\beta_0 = 2\\): 0 napsütéses óránál 2 cm az alapnövekedés
- \\(R^2 = 0.72\\): a napsütés a növekedés varianciájának 72%-át magyarázza, a maradék 28% más tényezőknek tudható be (pl. öntözés, talaj)
- Ha x=6 órára konfidenciaintervallum: [10.5, 11.5] cm — az átlagos növekedés ennyi napsütésnél ebben a tartományban van
- Ha x=6 órára predikciós intervallum: [8, 14] cm — egy konkrét kaktusz növekedése ennél szélesebb tartományban mozog
